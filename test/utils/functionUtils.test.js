...
describe("Function Utils", function () {
  ...
  beforeEach(() => {
    ...
    // Clear user cache
    Object.keys(userAccountCache).forEach(
      (key) => delete userAccountCache[key]
    );
  ...
  describe("fetchWithRetry", () => {
    ...
    it("should use default max retries", async () => {
      ...
      const response = await fetchWithRetry(
        "https://example.com/default-retries",
        {}
      );
  ...
  describe("lookupJiraUsers", () => {
    ...
    it("should lookup and cache Jira users", async () => {
      ...
      expect(userAccountCache).to.have.property(
        "test1@example.com",
        "account1"
      );
      expect(userAccountCache).to.have.property(
      ...
      projectsConfig.projects = [
        {
          key: "project1",
          name: "Project 1",
          assignee: "duplicate@example.com",
          isChecked: true,
        },
      ];
