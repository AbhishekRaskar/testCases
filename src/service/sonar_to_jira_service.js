...
exports.fetchSonarData = async (projectList = []) => {
  ...
  try {
    ...
    if (projectList && projectList.length > 0) {
      logger.info(
        `📋 Using provided project list with ${projectList.length} projects`
      );
      enabledProjects = projectList;
    } else {
      logger.info(
        "📂 No project list provided, loading from projectConfig.json..."
      );
      projects = projectsConfig.projects || [];
      enabledProjects = projects.filter((project) => project.isChecked);
      logger.info(
        `📊 Found ${enabledProjects.length} enabled projects out of ${projects.length} total`
      );
    }
    ...
    if (enabledProjects.length === 0) {
      logger.warn(
        "⚠️  No enabled projects found in configuration, checking environment fallback..."
      );
      const sonarProjectKey = envVars.SONARQUBE_PROJECT_KEY;

      if (!sonarProjectKey) {
        logger.warn(
          "🚫 No SONARQUBE_PROJECT_KEY environment variable defined. Returning empty results."
        );
      ...
      logger.info(
        `🚀 Processing ${enabledProjects.length} enabled projects...`
      );
    ...
    logger.error(
      "💥 Critical error during SonarQube data fetch",
...
exports.createJiraTickets = async (sonarData) => {
  ...
  logger.info(
    `📊 Processing ${totalItems} items (${issues.length} issues, ${hotspots.length} hotspots)`
  );
  ...
  try {
    ...
    logger.success(
      `✅ Jira user lookup completed (${
        Object.keys(userAccountCache).length
      } users cached)`
    );
    ...
    if (allSonarKeys.length > 0) {
      try {
        logger.info(
          `🔍 Checking for existing Jira tickets (${allSonarKeys.length} SonarQube keys)...`
        );
        ...
        for (let i = 0; i < allSonarKeys.length; i += maxBatchSize) {
          ...
          logger.progress(
            batchNum,
            totalBatches,
            `Checking existing tickets batch`
          );

          const jqlQuery = `project = ${jiraDefaultProject} AND (("Sonar Reference Key[Paragraph]" ~ "${batch.join(
            '") OR ("Sonar Reference Key[Paragraph]" ~ "'
          )}")) AND status NOT IN (Closed, Resolved, Done, Verified)`;
          ...
          if (searchData.issues && searchData.issues.length > 0) {
            logger.debug(
              `🔍 Found ${searchData.issues.length} existing tickets in batch ${batchNum}`
            );
            ...
            for (const ticket of searchData.issues) {
              try {
                ...
                logger.warn(
                  `⚠️  Could not parse Sonar key from ticket ${ticket.key}`,
                  e.message
                );
              }
            }
        ...
        logger.success(
          `✅ Existing ticket check completed (${existingTickets.length} existing tickets found)`
        );
      } catch (error) {
        logger.error(
          "❌ Error during batch ticket existence check",
          error.message
        );
      }
    }
    ...
    const createJiraTicket = async (item, itemType) => {
      ...
      if (existingTicket) {
        logger.debug(
          `🔄 Skipping ${itemType} ${sonarKey} - ticket already exists: ${existingTicket}`
        );
        return;
      }

      logger.info(
        `🆕 Creating new Jira ticket for ${itemType}: ${item.message.substring(
          0,
          50
        )}...`
      );
      ...
      const descriptionItems = [
          ...
          content: [
            {
              ...
              content: [
                {
                  type: "text",
                  text: `Project: ${projectInfo.name} (${projectKey})`,
                },
              ],
            },
          ],
        ...
        // Issue message
        {
          ...
          content: [
            {
              type: "paragraph",
              content: [{ type: "text", text: `Message: ${item.message}` }],
            },
          ],
        },
        // Affected file
        ...
        content: [
          {
            ...
            content: [
              ...
              {
                type: "text",
                text: `View ${itemType === "issue" ? "Issue" : "Hotspot"}`,
                marks: [{ type: "link", attrs: { href: sonarPermalink } }],
              },
            ],
          },
        ],
        ...
        content: [
          {
            ...
            content: [
              {
                type: "text",
                text: `${itemType === "issue" ? "Issue" : "Hotspot"} Details`,
              },
            ],
          },
          {
      ...
      if (itemType === "issue") {
        ...
        if (
          item.vulnerabilityProbability === "HIGH" ||
          item.vulnerabilityProbability === "MEDIUM"
        ) {
          priorityName = "Highest";
        } else {
          priorityName = "High";
      ...
      if (
        !jiraPayload.fields.summary ||
        jiraPayload.fields.summary.trim() === ""
      ) {
      ...
      logger.debug(`📋 Creating Jira ticket with payload:`, {
        ...
        assignee: jiraPayload.fields.assignee
          ? `${
              jiraPayload.fields.assignee.accountId ||
              jiraPayload.fields.assignee.name
            }`
          : "None",
        components:
          jiraPayload.fields.components?.map((c) => c.name).join(", ") ||
          "None",
      });
      ...
      if (projectInfo.assignee) {
        ...
        if (cachedUser) {
          ...
          logger.debug(
            `👤 Assigning ticket to ${projectInfo.assignee} (accountId: ${cachedUser})`
          );
        } else {
          // Don't assign if user not found in cache - let Jira handle default assignment
          logger.warn(
            `⚠️  User ${projectInfo.assignee} not found in cache, skipping assignment`
          );
        }
      } else {
    ...
    // Process hotspots
    logger.info(
      `🔒 Processing ${hotspots.length} SonarQube security hotspots...`
    );
    ...
    for (const hotspot of hotspots) {
      ...
      logger.progress(
        processedHotspots,
        hotspots.length,
        `Processing hotspot ${hotspot.key}`
      );
      try {
 ...
 */
async function checkMultipleIssuesResolved(
  sonarKeys,
  sonarBaseUrl,
  sonarToken
) {
  logger.debug(
    `🔍 Batch checking ${sonarKeys.length} SonarQube items for resolution status...`
  );
  ...
  try {
    ...
    for (let i = 0; i < sonarKeys.length; i += maxBatchSize) {
      ...
      const issueApiUrl = `${sonarBaseUrl}/api/issues/search?issues=${batch.join(
        ","
      )}&resolved=false`;
      logger.debug(
        `📡 Checking issues API with ${batch.length} keys for unresolved issues...`
      );

      try {
        const issueResponse = await fetchWithRetry(issueApiUrl, {
          headers: {
            Authorization: `Basic ${Buffer.from(`${sonarToken}:`).toString(
              "base64"
            )}`,
            Accept: "application/json",
          },
        ...
        if (issueResponse.ok) {
          ...
          const foundIssueKeys = (issueData.issues || []).map(
            (issue) => issue.key
          );
          ...
          logger.info(
            `🔍 Found ${foundIssueKeys.length} active unresolved issues in batch`
          );
        } else {
          const errorText = await issueResponse.text();
          logger.warn(
            `⚠️ Issues API error: HTTP ${issueResponse.status} - ${errorText}`
          );
        }
      } catch (error) {
      ...
      const keysToCheckAsHotspots = batch.filter(
        (key) => resolvedStatus[key] === true
      );
      ...
      if (hotspotResolved.length > 0) {
        logger.debug(
          `✅ Resolved hotspots in batch: ${
            hotspotResolved.length
          } (${hotspotResolved.slice(0, 3).join(", ")}${
            hotspotResolved.length > 3 ? "..." : ""
          })`
        );
      }
      if (hotspotActive.length > 0) {
        logger.debug(
          `⚠️  Active hotspots in batch: ${
            hotspotActive.length
          } (${hotspotActive.slice(0, 3).join(", ")}${
            hotspotActive.length > 3 ? "..." : ""
          })`
        );
      }
      if (hotspotErrors.length > 0) {
        logger.warn(
          `❌ Hotspot errors in batch: ${hotspotErrors.length} (${hotspotErrors
            .slice(0, 2)
            .map((e) => `${e.key}:${e.status || "unknown"}`)
            .join(", ")}${hotspotErrors.length > 2 ? "..." : ""})`
        );
      }
    ...
    const resolvedCount = Object.values(resolvedStatus).filter(
      (status) => status === true
    ).length;
    const activeCount = Object.values(resolvedStatus).filter(
      (status) => status === false
    ).length;

    logger.success(
      `✅ Batch check completed: ${resolvedCount} resolved, ${activeCount} active, ${sonarKeys.length} total`
    );

    // Log detailed breakdown for troubleshooting
    const resolvedKeys = Object.keys(resolvedStatus).filter(
      (key) => resolvedStatus[key] === true
    );
    const activeKeys = Object.keys(resolvedStatus).filter(
      (key) => resolvedStatus[key] === false
    );

    if (resolvedKeys.length > 0) {
      logger.info(
        `✅ Resolved items: ${resolvedKeys.slice(0, 5).join(", ")}${
          resolvedKeys.length > 5
            ? ` ... and ${resolvedKeys.length - 5} more`
            : ""
        }`
      );
    }
    if (activeKeys.length > 0) {
      logger.info(
        `🔒 Active items: ${activeKeys.slice(0, 5).join(", ")}${
          activeKeys.length > 5 ? ` ... and ${activeKeys.length - 5} more` : ""
        }`
      );
    }
 ...
 */
async function closeJiraTicket(
  jiraTicketKey,
  sonarKey,
  jiraUsername,
  jiraApiToken,
  ...
  logger.debug(
    `🔒 Closing Jira ticket ${jiraTicketKey} for resolved SonarQube issue ${sonarKey}...`
  );
  ...
  try {
    ...
    if (!transitionId) {
      const availableTransitions = transitionsData.transitions
        ...
        .join(", ");
      logger.warn(
        `⚠️ Could not find a suitable transition for ticket ${jiraTicketKey}`,
        {
    ...
    if (!closeResponse.ok) {
      logger.warn(
        `⚠️ Transition with comment failed for ticket ${jiraTicketKey}, trying transition only...`
      );
      ...
      if (closeResponse.ok) {
        logger.debug(
          `✅ Transition succeeded, adding comment separately for ticket ${jiraTicketKey}...`
        );
        ...
        try {
          ...
          if (!commentResponse.ok) {
            logger.warn(
              `⚠️ Failed to add comment to ticket ${jiraTicketKey}, but transition succeeded`
            );
          }
        } catch (commentError) {
          logger.warn(
            `⚠️ Error adding comment to ticket ${jiraTicketKey}: ${commentError.message}`
          );
        }
      }
    }
...
exports.closeResolvedJiraTickets = async () => {
  ...
  logger.info(
    "🔄 Starting process to close Jira tickets for resolved SonarQube issues..."
  );
  ...
  try {
    ...
    while (hasMore) {
      try {
        ...
        await rateLimiter.wait();
        logger.debug(
          `🔍 Fetching tickets batch: startAt=${startAt}, maxResults=${maxResults}`
        );
        ...
        logger.progress(
          (Math.min(startAt, totalTickets) / totalTickets) * 100,
          100,
          `Fetched ${Math.min(
            startAt,
            totalTickets
          )}/${totalTickets} tickets (${Math.round(
            (Math.min(startAt, totalTickets) / totalTickets) * 100
          )}%)`
        );
      } catch (error) {
        ...
        if (allTickets.length > 0) {
          logger.warn(
            `⚠️ Pagination error encountered, but will continue processing ${allTickets.length} tickets already retrieved`
          );
          hasMore = false;
        } else {
          throw error;
    ...
    logger.success(
      `✅ Pagination complete. Total tickets retrieved: ${allTickets.length}`
    );
    ...
    allTickets.forEach((ticket) => {
      try {
        ...
        if (customField?.content?.length > 0) {
          ...
          if (paragraph.content && paragraph.content.length > 0) {
            ...
            if (sonarKey) {
              ...
              logger.debug(
                `✅ Extracted SonarQube key: ${sonarKey} from ticket: ${ticket.key}`
              );
            } else {
            ...
            logger.warn(
              `⚠️ No paragraph content found in ticket ${ticket.key}`,
              {
          ...
          logger.warn(
            `⚠️ No custom field content found in ticket ${ticket.key}`,
            {
              hasCustomField: !!customField,
        ...
        logger.warn(
          `⚠️ Could not extract Sonar key from ticket ${ticket.key}`,
          e.message
        );
      }
    });
    ...
    logger.success(
      `✅ Extracted ${allSonarKeys.length} SonarQube keys from ${allTickets.length} tickets`
    );

    if (allSonarKeys.length === 0) {
      logger.warn(
        "⚠️ No SonarQube keys found in any tickets. Nothing to process."
      );
      return {
    ...
    if (allSonarKeys.length > 0) {
      logger.info(
        `🔍 Batch checking ${allSonarKeys.length} SonarQube items for resolution status...`
      );
      resolvedStatusMap = await checkMultipleIssuesResolved(
        allSonarKeys,
        sonarBaseUrl,
        envVars.SONARQUBE_TOKEN
      );
    }
    ...
    for (let i = 0; i < allTickets.length; i += maxConcurrent) {
          ...
          if (customField?.content?.length > 0) {
            ...
            if (paragraph.content && paragraph.content.length > 0) {
              ...
              if (isResolved === true) {
                // Close the ticket
                await rateLimiter.wait();
                const closed = await closeJiraTicket(
                  ticket.key,
                  sonarKey,
                  jiraUsername,
                  jiraApiToken,
                ...
                if (closed) {
                  ...
                  logger.debug(
                    `✅ Successfully closed ticket ${ticket.key} for resolved issue ${sonarKey}`
                  );
                } else {
                  errorCount++;
                  processedTickets.errors.push(ticket.key);
                  logger.error(
                    `❌ Failed to close ticket ${ticket.key} for resolved issue ${sonarKey}`
                  );
                }
                ...
                logger.warn(
                  `⚠️ Unknown resolution status for SonarQube item ${sonarKey}, skipping ticket ${ticket.key}`
                );
                errorCount++;
                processedTickets.errors.push(ticket.key);
              }
              ...
              logger.warn(
                `⚠️ Could not extract paragraph content from ticket ${ticket.key}`,
                {
                  contentLength: paragraph.content
      ...
      if (
        (i + batch.length) % 50 === 0 ||
        i + batch.length >= allTickets.length
      ) {
        logger.info(
          `📊 Processed ${i + batch.length}/${
            allTickets.length
          } tickets (${Math.round(
            ((i + batch.length) / allTickets.length) * 100
          )}%) - Closed: ${closedCount}, Errors: ${errorCount}, Not Resolved: ${notResolvedCount}`
        );
      }
    }
    ...
    if (processedTickets.closed.length > 0) {
      logger.info(
        `✅ Closed tickets (${
          processedTickets.closed.length
        }): ${processedTickets.closed.slice(0, 20).join(", ")}${
          processedTickets.closed.length > 20
            ? ` ... and ${processedTickets.closed.length - 20} more`
            : ""
        }`
      );
    }
    if (processedTickets.errors.length > 0) {
      logger.warn(
        `❌ Error tickets (${
          processedTickets.errors.length
        }): ${processedTickets.errors.slice(0, 20).join(", ")}${
          processedTickets.errors.length > 20
            ? ` ... and ${processedTickets.errors.length - 20} more`
            : ""
        }`
      );
    }
    if (processedTickets.notResolved.length > 0) {
      logger.info(
        `⏳ Not resolved tickets (${
          processedTickets.notResolved.length
        }): ${processedTickets.notResolved.slice(0, 20).join(", ")}${
          processedTickets.notResolved.length > 20
            ? ` ... and ${processedTickets.notResolved.length - 20} more`
            : ""
        }`
      );
    }
