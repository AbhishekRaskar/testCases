...
exports.sonarToJiraHandler = async (event, context) => {
  ...
  try {
      ...
      try {
        ...
        if (fetcherResult.StatusCode === 200) {
          ...
          if (fetcherPayload.statusCode === 200) {
            const body =
              typeof fetcherPayload.body === "string"
            ...
            if (projectList.length > 10) {
              logger.warn(
                `⚡ Large project count detected (${projectList.length} projects). Switching to batch processing mode...`
              );
              return await processBatchedProjects(projectList);
            }
          } else {
            logger.error(
              "❌ Project Fetcher Lambda returned error response",
              fetcherPayload
            );
          }
        } else {
          logger.error(
            `❌ Project fetcher Lambda failed with status code: ${fetcherResult.StatusCode}`
          );
        }
      } catch (lambdaError) {
        logger.error(
          "🔧 Project Fetcher Lambda invocation failed, falling back to direct SonarQube fetch",
          lambdaError.message
        );
      }
    } else {
      logger.info(
        "🏠 Running in local environment, skipping Lambda invocation"
      );
    }
    ...
    const createdTickets = await sonarToJiraService.createJiraTickets(
      sonarIssues
    );
    ...
    logger.error(
      "💥 Critical error in SonarQube to Jira integration",
      error.message
    );
...
async function processBatchedProjects(projectList) {
  ...
  logger.info(
    `🔄 Starting batch processing: ${projectList.length} projects across ${totalBatches} batches`
  );
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    logger.progress(
      i + 1,
      totalBatches,
      `Processing batch with ${batch.length} projects`
    );
    logger.debug(
      `📦 Batch ${i + 1} contains projects:`,
      batch.map((p) => p.key)
    );
...
function formatResponse(createdTickets) {
  ...
  logger.info(`${emoji} Final result: ${message}`, {
    summary: {
      ...
      totalProcessed:
        createdTickets.created.length + createdTickets.existing.length,
    },
  });
