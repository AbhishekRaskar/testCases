...
async function processSingleProject(projectKey, sonarIssueUrl, sonarHotspotUrl, sonarToken, sonarPassword, allIssues, allHotspots) {
    ...
    try {
        ...
        const issueResponse = await fetchWithRetry(`${sonarIssueUrl}?${issueParams.toString()}`, {
            headers: {
                'Authorization': `Basic ${Buffer.from(`${sonarToken}:${sonarPassword}`).toString('base64')}`,
                'Accept': 'application/json',
            },
        });

        if (!issueResponse.ok) {
            throw new Error(`HTTP error ${issueResponse.status}: ${await issueResponse.text()}`);
        }
        ...
        if (issues.length > 0) {
            logger.success(`✅ Found ${issues.length} issues for project ${projectKey}`);
    ...
    try {
        ...
        const hotspotResponse = await fetchWithRetry(`${sonarHotspotUrl}?${hotspotParams.toString()}`, {
            headers: {
                'Authorization': `Basic ${Buffer.from(`${sonarToken}:${sonarPassword}`).toString('base64')}`,
                'Accept': 'application/json',
            },
        ...
        if (!hotspotResponse.ok) {
            throw new Error(`HTTP error ${hotspotResponse.status}: ${await hotspotResponse.text()}`);
        }
        ...
        if (hotspots.length > 0) {
            logger.success(`🔒 Found ${hotspots.length} security hotspots for project ${projectKey}`);
            allHotspots.push(...hotspots);
        } else {
            logger.info(`ℹ️  No security hotspots found for project ${projectKey}`);
        }
    } catch (error) {
        logger.error(`❌ Failed to fetch hotspots for project ${projectKey}`, error.message);
    }
...
async function lookupJiraUsers(jiraBaseUrl, jiraUsername, jiraApiToken) {
    ...
    logger.info(`📧 Found ${uniqueEmails.length} unique assignee emails to lookup`);
    ...
    for (const email of uniqueEmails) {
        ...
        try {
            ...
            const userSearchUrl = `${jiraBaseUrl}/rest/api/3/user/search?query=${encodeURIComponent(email)}`;
            ...
            if (userResponse.ok) {
                ...
                logger.error(`❌ Error looking up user ${email}: ${await userResponse.text()}`);
            }
        } catch (error) {
            logger.error(`💥 Exception while looking up user ${email}`, error.message);
        }
    }
